<template>
  <div class="min-h-screen bg-background flex flex-col justify-center items-center px-4 font-sans relative overflow-x-hidden">
    ...
    <!-- Logo Image -->
    <div class="absolute top-16 sm:top-20 left-1/2 -translate-x-1/2 w-40 sm:w-48 z-0">
      <img src="@/assets/logo-persegi.png" alt="Logo" class="w-full h-auto object-contain drop-shadow-xl" />
    </div>

    <!-- Main Card matching reference style but with brown palette -->
    <div class="w-full max-w-md space-y-5 bg-[#E9D8C6] text-[#4B2E2A] p-6 rounded-2xl border-2 border-[#4B2E2A] shadow-[8px_8px_0px_#4B2E2A] relative z-10">
      
      <!-- Header Section -->
      <div class="text-center space-y-2">
        <h2 class="text-3xl font-extrabold font-heading tracking-wide">
          Selamat Datang
        </h2>
        <p class="text-xs sm:text-sm font-medium leading-relaxed opacity-90 px-1">
          Silahkan meminta bantuan pelayan atau langsung input kode outlet
        </p>
      </div>
      
      <!-- Form Section -->
      <form class="space-y-4" @submit.prevent="validateCode">
        <div>
          ...
          <div class="relative">
            ...
            <input 
              ...
              required 
              maxlength="6"
              class="appearance-none block w-full pl-11 pr-4 py-3 border-2 rounded-xl text-gray-900 bg-white placeholder-gray-400 text-base font-bold uppercase tracking-widest focus:outline-none transition-all duration-300"
              :class="[
                isError 
          ...
          <p v-if="isError" class="mt-2 text-center text-xs font-bold text-red-500 animate-pulse">
            {{ errorMessage }}
          </p>
        </div>
        ...
        <div>
          <button 
            ...
            class="group relative w-full flex justify-center py-3.5 px-4 border border-transparent text-base font-bold rounded-xl text-[#F7F2EC] bg-[#4B2E2A] hover:bg-[#7A4A3A] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#4B2E2A] transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:active:scale-100 disabled:cursor-not-allowed shadow-md"
          >
